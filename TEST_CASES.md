...
| TC-AUTH-002 | Registration fails with duplicate email — system blocks registration if email is already in use | 1. Attempt to register using an already-registered email | Email: existing@email.com (already registered) | Error shown: email already in use, registration does not proceed | Alert shown: "Email is already registered", registration blocked | Pass |
| TC-AUTH-003 | Registration fails with duplicate phone number — system blocks registration if phone number is already in use | 1. Attempt to register using an already-registered phone number | Phone: 09171234567 (already registered) | Error shown: phone number already in use | Alert shown: "Phone number is already registered", registration blocked (requires server migration to apply @unique constraint) | Pass |
| TC-AUTH-004 | Registration fails with weak password — system enforces password complexity requirements | 1. Enter a weak password 2. Tap "Create Account" | Password: password123 (no uppercase or special character) | Error: password must have uppercase, lowercase, number, special character, min 8 characters | In-line password requirement indicators turn red, alert shown if submitted with failing password | Pass |
| TC-AUTH-005 | Registration blocked without accepting Terms & Privacy — Create Account button is disabled until both checkboxes are checked | 1. Fill in all fields correctly 2. Leave both checkboxes unchecked 3. Observe Create Account button | All valid fields, checkboxes unchecked | "Create Account" button is disabled and cannot be tapped | Button appears grayed out and is non-interactive until both checkboxes are checked | Pass |
| TC-AUTH-006 | Successful login — a verified user can log in with correct credentials | 1. Enter correct email and password 2. Tap "Log In" | Email: juan@email.com, Password: Juan@1234 | User logged in and redirected to their role's home screen | Login successful, JWT tokens stored, user redirected to correct home screen based on role | Pass |
| TC-AUTH-007 | Login fails with wrong password — system rejects login with incorrect password | 1. Enter correct email but wrong password 2. Tap "Log In" | Email: juan@email.com, Password: WrongPass1! | Error: invalid credentials | Alert shown: "Invalid credentials", login blocked | Pass |
| TC-AUTH-008 | Email verification flow — user can verify account using the code sent to their email | 1. Check email for verification code 2. Enter code on verification screen 3. Tap "Verify" | Verification code from email inbox | Email verified, user proceeds to role selection or home screen | Code accepted, email marked verified, user auto-logged in and redirected to role selection screen | Pass |
| TC-AUTH-009 | Forgot password — user can request a password reset link via email | 1. Tap "Forgot Password" on login screen 2. Enter registered email address 3. Tap "Send Reset Link" | Email: juan@email.com (registered account) | Password reset email sent, success message shown on screen | Success alert shown: "If your email is registered, you will receive a password reset link", redirected to reset password screen | Pass |
| TC-AUTH-010 | Forgot password fails with unregistered email — system does not reveal whether email exists | 1. Tap "Forgot Password" 2. Enter an email not linked to any account 3. Tap "Send Reset Link" | Email: notregistered@email.com | Generic message shown (e.g. "If this email is registered, a reset link has been sent"), no account details leaked | Same generic success message shown regardless of whether email is registered, no account info leaked | Pass |
| TC-AUTH-011 | Password reset code received and entered — after requesting a reset, user receives a 6-digit code via email, opens the reset screen, and submits the code along with a new password | 1. Open password reset email 2. Copy the 6-digit reset code 3. Open app and navigate to reset password screen 4. Enter registered email, reset code, new password, and confirm password 5. Tap submit | 6-digit reset code from email, New password: NewPass@5678, Confirm: NewPass@5678 | Password updated successfully, user redirected to login screen | 6-digit code accepted, password updated, alert shown, user redirected to login screen | Pass |
| TC-AUTH-012 | Password reset code expires after timeout — expired code is rejected | 1. Request a reset code 2. Wait more than 5 minutes 3. Attempt to submit the expired code | Reset code older than 5 minutes | Error shown: reset code expired, user prompted to request a new one | Alert shown: "Reset code expired", password not changed | Pass |
| TC-AUTH-013 | User successfully resets password — new password is accepted and user can log in | 1. Open valid reset link 2. Enter new password and confirm 3. Submit | New password: NewPass@5678, Confirm: NewPass@5678 | Password updated, user redirected to login screen, can log in with new password | Password updated in database, user can log in with new password, old password no longer works | Pass |
| TC-AUTH-014 | Password reset fails when new passwords do not match — form validation blocks mismatched entries | 1. Open valid reset link 2. Enter mismatched new password and confirm password 3. Submit | New password: NewPass@5678, Confirm: Different@999 | Error shown: passwords do not match, password not updated | Alert shown: "Passwords do not match", form not submitted, password unchanged | Pass |
| TC-AUTH-015 | Change password — logged-in user can change their password from profile settings | 1. Go to profile settings 2. Tap "Change Password" 3. Enter current password 4. Enter new password and confirm 5. Submit | Current password: Juan@1234, New password: Updated@5678, Confirm: Updated@5678 | Password updated successfully, user notified, must use new password on next login | Alert shown: "Password changed successfully", new password required on next login | Pass |
| TC-AUTH-016 | Change password fails with wrong current password — system rejects incorrect current password | 1. Go to profile settings 2. Tap "Change Password" 3. Enter wrong current password 4. Submit | Current password: WrongPass1! (incorrect) | Error: current password is incorrect, password not changed | Alert shown: "Current password is incorrect", password not changed | Pass |
...
| Test Case ID | Test Case Description | Test Steps | Test Data | Expected Results | Actual Results | Pass/Fail |
|---|---|---|---|---|---|---|
| TC-BOOK-001 | Driver successfully books a parking spot — a verified driver with sufficient balance can complete a booking | 1. Open map 2. Select a location 3. Tap "View Details" 4. Select space and vehicle 5. Tap "Pay & Book Now" 6. Confirm | Driver: verified, Wallet balance: ₱200, Hourly rate: ₱50 | Booking created (PENDING), first-hour fee deducted from wallet, host notified | | |
| TC-BOOK-002 | Booking blocked when wallet is insufficient — driver cannot book if balance is below the first-hour fee | 1. Navigate to a parking spot booking screen | Driver wallet balance: ₱10, Hourly rate: ₱50 | Book button disabled, shows "Insufficient Balance", cannot proceed | | |
| TC-BOOK-003 | Booking blocked when driver has outstanding balance — driver with PAYMENT_PENDING reservation cannot make new bookings | 1. Navigate to any parking spot booking screen | Driver has 1 existing PAYMENT_PENDING reservation | Book button disabled, shows "Settle Outstanding Balance First" | | |
| TC-BOOK-004 | Low balance warning shown for 1-hour coverage — orange banner shown when wallet only covers 1 hour | 1. Navigate to booking screen with wallet covering exactly 1 hour | Driver wallet: ₱50, Hourly rate: ₱50 | Orange warning banner shown: booking credit is only for 1 hour | | |
| TC-BOOK-005 | Host approves a booking — host can approve a pending booking request | 1. Host opens booking from dashboard 2. Taps "Approve" 3. Confirms | Booking in PENDING status | Booking → CONFIRMED, driver receives approval notification with 60-min arrival window | | |
| TC-BOOK-006 | Host rejects a booking — host can reject a pending booking and escrow is refunded | 1. Host opens booking 2. Taps "Reject" 3. Confirms | Booking in PENDING status | Booking cancelled, escrow refunded to driver wallet, driver notified of rejection | | |
| TC-BOOK-007 | Booking auto-expires when driver does not arrive — CONFIRMED booking expires after 1-hour arrival window | 1. Allow arrival deadline to pass without driver arriving | CONFIRMED booking, 1 hour elapsed since approval | Booking → EXPIRED, escrow forfeited, driver notified "Booking Expired", host notified "Driver Did Not Arrive", slot freed | | |
| TC-BOOK-008 | Driver cancels booking before arrival — driver can cancel a confirmed booking before arriving | 1. Driver opens reservation 2. Taps cancel 3. Confirms | Booking in CONFIRMED status | Booking cancelled, host notified, parking slot freed | | |
...
| Test Case ID | Test Case Description | Test Steps | Test Data | Expected Results | Actual Results | Pass/Fail |
|---|---|---|---|---|---|---|
| TC-NOTIF-001 | Driver receives booking approved notification — driver is notified via push and in-app when host approves | 1. Host approves a pending booking | Driver has PENDING booking, notifications enabled | Driver receives push + in-app "Booking Approved" notification | | |
| TC-NOTIF-002 | Both parties notified when driver is approaching (500m) — single notification sent to driver and host | 1. Driver navigates toward location 2. Enter within 500m of spot | CONFIRMED booking, driver GPS active, driver within 500m | Driver: "Almost There!", Host: "Driver Approaching". Fires once per reservation | | |
| TC-NOTIF-003 | Both parties notified when driver arrives (12m) — arrival notification sent to driver and host | 1. Driver enters within 12m of location | CONFIRMED booking, driver GPS active, driver within 12m | Driver: "You Have Arrived!", Host: "Driver Has Arrived". Fires once only | | |
| TC-NOTIF-004 | Approaching notification does not fire multiple times — server dedup prevents repeat sends | 1. Driver enters 500m zone (notification fires) 2. Driver exits 500m zone 3. Driver re-enters 500m zone | Driver already has DRIVER_NEARBY notification record for reservation | No duplicate notification sent — server dedup check prevents repeat | | |
| TC-NOTIF-005 | Admin notified for new top-up request — all admins receive notification when driver submits top-up | 1. Driver submits a top-up request | Active admin accounts exist, top-up amount: ₱500 | All admins receive "New Top-Up Request" push + in-app notification | | |
| TC-NOTIF-006 | Admin notified for new withdrawal request — all admins receive notification when user requests withdrawal | 1. User submits a withdrawal request | Active admin accounts exist, withdrawal amount: ₱200 | All admins receive "New Withdrawal Request" push + in-app notification | | |
