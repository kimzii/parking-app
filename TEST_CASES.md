...
| TC-AUTH-009 | Forgot password — user can request a password reset code via email | 1. Tap "Forgot Password" on login screen 2. Enter registered email address 3. Tap "Send Reset Code" | Email: juan@email.com (registered account) | 6-digit reset code sent to email, success message shown on screen, redirected to reset password screen | Success alert shown: "If your email is registered, you will receive a 6-digit reset code.", user redirected to reset password screen | Pass |
| TC-AUTH-010 | Forgot password fails with unregistered email — system does not reveal whether email exists | 1. Tap "Forgot Password" 2. Enter an email not linked to any account 3. Tap "Send Reset Code" | Email: notregistered@email.com | Generic success message shown regardless of whether email exists, no account details leaked | Same generic success message shown for both registered and unregistered emails, no info leaked | Pass |
| TC-AUTH-011 | Password reset code received and entered — user opens email, copies the 6-digit code, and submits it on the reset screen along with a new password | 1. Open password reset email 2. Copy the 6-digit reset code 3. Open app and navigate to reset password screen 4. Enter registered email, reset code, new password, and confirm password 5. Tap "Reset Password" | 6-digit reset code from email, New password: NewPass@5678, Confirm: NewPass@5678 | Password updated successfully, user redirected to login screen | 6-digit code accepted, password updated, alert shown, user redirected to login screen | Pass |
| TC-AUTH-012 | Password reset code expires after 5 minutes — expired code is rejected | 1. Request a reset code 2. Wait more than 5 minutes 3. Navigate to reset password screen 4. Enter the expired code and new password 5. Submit | Reset code older than 5 minutes | Error shown: reset code expired, user prompted to request a new one | Alert shown: "Reset code expired", password not changed | Pass |
| TC-AUTH-013 | User successfully resets password — valid code accepted, new password saved, user can log in | 1. Request reset code via Forgot Password 2. Open reset password screen 3. Enter email, valid code, new password, and confirm 4. Tap "Reset Password" | Valid 6-digit code, New password: NewPass@5678, Confirm: NewPass@5678 | Password updated, user redirected to login screen, can log in with new password | Password updated in database, user can log in with new password, old password no longer works | Pass |
| TC-AUTH-014 | Password reset fails when new passwords do not match — form validation blocks mismatched entries | 1. Navigate to reset password screen 2. Enter valid code 3. Enter mismatched new password and confirm password 4. Tap "Reset Password" | Valid 6-digit code, New password: NewPass@5678, Confirm: Different@999 | Error shown: passwords do not match, password not updated | Alert shown: "Passwords do not match", form not submitted, password unchanged | Pass |
