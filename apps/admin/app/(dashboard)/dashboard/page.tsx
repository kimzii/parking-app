...
const StatCard = ({
  ...
  <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-between h-[155px]">
    ...
    <div className="mt-2 pt-3 border-t border-gray-50 flex items-center gap-2 text-xs font-medium">
      ...
      <span className={footerColor}>{footerLabel}</span>
    </div>
  </div>
);
...
export default function DashboardPage() {
  ...
  const [selectedMapListing, setSelectedMapListing] =
    useState<MappableListing | null>(null);
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
          ...
          for (let page = 2; page <= totalPages; page += 1) {
            pageRequests.push(
              api.get<AdminListingsResponse>(
                `/hosts/admin/locations?page=${page}&limit=${limit}`,
              ),
            );
          }
      ...
      try {
        setLoading(true);
        const [statsRes, listingsRes, activityRes, allListingsRes] =
          await Promise.all([
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-50 flex justify-between items-center">
              ...
              <Link
                href="/listings"
                className="text-sm text-[#C94B1E] font-medium hover:underline"
              >
                View All
              </Link>
            </div>
                  ...
                  {recentListings.length === 0 ? (
                    ...
                    recentListings.map((listing) => (
                      <tr
                        ...
                        onClick={() =>
                          router.push(
                            `/listings?listingId=${encodeURIComponent(listing.id)}`,
                          )
                        }
                        onKeyDown={(event) => {
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-6 border-b border-gray-50 flex justify-between items-center">
              ...
              <span className="text-xs text-gray-500">
                {mappableListings.length} pinned locations
              </span>
            </div>
            <div className="h-[340px]">
              {!googleMapsApiKey ? (
                <div className="h-full flex items-center justify-center text-sm text-gray-500 px-6 text-center">
                  Google Maps key is missing. Set
                  NEXT_PUBLIC_GOOGLE_MAPS_API_KEY in admin .env.local.
                </div>
              ) : mappableListings.length === 0 ? (
                <div className="h-full flex items-center justify-center text-sm text-gray-500 px-6 text-center">
                ...
                <div className="h-full flex items-center justify-center text-sm text-red-600 px-6 text-center">
                  Failed to load Google Maps. Please verify the API key and
                  allowed referrers.
                </div>
              ) : !isMapLoaded ? (
                <div className="h-full flex items-center justify-center text-sm text-gray-500 px-6 text-center">
                ...
                <div className="w-full max-w-md rounded-xl bg-white shadow-xl border border-gray-100">
                  ...
                  <div className="px-5 py-4 space-y-2">
                    ...
                    <p className="text-sm text-gray-600">
                      {selectedMapListing.address}
                    </p>
                    <p className="text-xs text-gray-500">
                      Host: {selectedMapListing.hostName}
                    </p>
                    <p className="text-xs text-gray-500">
                      Status: {selectedMapListing.status}
                    </p>
                  </div>
                  ...
                  <div className="px-5 py-4 border-t border-gray-100 flex items-center justify-end gap-2">
                    ...
                    <button
                      onClick={() => {
                        router.push(
                          `/listings?listingId=${selectedMapListing.id}`,
                        );
